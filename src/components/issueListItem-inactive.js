...
import { findBidMean, findBidMedian, findBidMode, findBidsByValue } from '../utility/bidMaths';
...
function InactiveItem({ item }) {
  ...
  const renderBidSums = () => {
    ...
    return bidsByBidValue.map(({ bidValue, bids = [] }) => (
      <Tooltip key={bidValue} title={bids.map(bid => (<div key={bid.bidderName}>{bid.bidderName}</div>))}>
